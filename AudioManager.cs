...
namespace Sharp
{
    public static class AudioManager
    {
        ...
        public static void Init(bool isWasapiExclusive = false)
        {
            ...
            Bass.BASS_Init(deviceId, hertz, BASSInit.BASS_DEVICE_LATENCY, IntPtr.Zero);
            ...
            if (nDevNo != -1)
            {
                Interval = (long)((double)deviceInfo.minperiod * 1000.0);
            ...
            if (BassWasapi.BASS_WASAPI_Init(deviceId, hertz, channel, flags, (float)DesiredBufferSize / 1000f, (float)Interval / 1000f, _wasapiProc, IntPtr.Zero))
            {
                if (isWasapiExclusive)
                {
                    ...
                    RBufferSize = (long)((float)wasapiInfo.buflen * 1000f / (float)onescByte);
                    RDelaySize = 0L;
                }
                else
                {
